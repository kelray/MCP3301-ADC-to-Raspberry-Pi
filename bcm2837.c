...
// $Id: bcm2837.c,v 1.21 2015/03/08 23:13:25 mikem Exp mikem $
*/
...
*/
uint32_t *bcm2837_peripherals_base = (uint32_t *)BCM2837_PERI_BASE;
uint32_t bcm2837_peripherals_size = BCM2837_PERI_SIZE;

/* Virtual memory address of the mapped peripherals block 
 */
uint32_t *bcm2837_peripherals = (uint32_t *)MAP_FAILED;

/* And the register bases within the peripherals block
 */
volatile uint32_t *bcm2837_gpio        = (uint32_t *)MAP_FAILED;
volatile uint32_t *bcm2837_pwm         = (uint32_t *)MAP_FAILED;
volatile uint32_t *bcm2837_clk         = (uint32_t *)MAP_FAILED;
volatile uint32_t *bcm2837_pads        = (uint32_t *)MAP_FAILED;
volatile uint32_t *bcm2837_spi0        = (uint32_t *)MAP_FAILED;
volatile uint32_t *bcm2837_bsc0        = (uint32_t *)MAP_FAILED;
volatile uint32_t *bcm2837_bsc1        = (uint32_t *)MAP_FAILED;
volatile uint32_t *bcm2837_st	       = (uint32_t *)MAP_FAILED;
...
uint32_t* bcm2837_regbase(uint8_t regbase)
{
    switch (regbase)
    {
	case BCM2837_REGBASE_ST:
	    return (uint32_t *)bcm2837_st;
	case BCM2837_REGBASE_GPIO:
	    return (uint32_t *)bcm2837_gpio;
	case BCM2837_REGBASE_PWM:
	    return (uint32_t *)bcm2837_pwm;
	case BCM2837_REGBASE_CLK:
	    return (uint32_t *)bcm2837_clk;
	case BCM2837_REGBASE_PADS:
	    return (uint32_t *)bcm2837_pads;
	case BCM2837_REGBASE_SPI0:
	    return (uint32_t *)bcm2837_spi0;
	case BCM2837_REGBASE_BSC0:
	    return (uint32_t *)bcm2837_bsc0;
	case BCM2837_REGBASE_BSC1:
	    return (uint32_t *)bcm2837_st;
    }
    return (uint32_t *)MAP_FAILED;
}

void  bcm2837_set_debug(uint8_t d)
{
    debug = d;
}

unsigned int bcm2837_version(void) 
{
    return BCM2837_VERSION;
}
...
uint32_t bcm2837_peri_read(volatile uint32_t* paddr)
{
    if (debug)
    {
        printf("bcm2837_peri_read  paddr %08X\n", (unsigned) paddr);
	return 0;
    }
    else
    {
...
" : [ret] "=r" (ret) : [paddr] "r" (paddr) : "r10", "memory" );
       ...
       return ret;
#else
       return *paddr; /* Not used on bcm2837 */
#endif
...
uint32_t bcm2837_peri_read_nb(volatile uint32_t* paddr)
{
    if (debug)
    {
	printf("bcm2837_peri_read_nb  paddr %08X\n", (unsigned) paddr);
	return 0;
    }
    else
    {
...
void bcm2837_peri_write(volatile uint32_t* paddr, uint32_t value)
{
    if (debug)
    {
	printf("bcm2837_peri_write paddr %08X, value %08X\n", (unsigned) paddr, value);
    }
    else
    {
#ifdef __arm__
...
void bcm2837_peri_write_nb(volatile uint32_t* paddr, uint32_t value)
{
    if (debug)
    {
	printf("bcm2837_peri_write_nb paddr %08X, value %08X\n",
               (unsigned) paddr, value);
    }
    else
    {
 ...
 */
void bcm2837_peri_set_bits(volatile uint32_t* paddr, uint32_t value, uint32_t mask)
{
...
{
    volatile uint32_t* paddr = bcm2837_pads + BCM2837_PADS_GPIO_0_27/4 + group*2;
...
// milliseconds
*/
void bcm2837_delay(unsigned int millis)
{
...
/* microseconds */
void bcm2837_delayMicroseconds(uint64_t micros)
{
    struct timespec t1;
    uint64_t        start;
	...
	printf("bcm2837_delayMicroseconds %lld\n", micros);
	return;
    }
...
void bcm2837_gpio_write(uint8_t pin, uint8_t on)
{
    if (on)
...
void bcm2837_gpio_write_multi(uint32_t mask, uint8_t on)
{
    if (on)
	bcm2837_gpio_set_multi(mask);
    else
	bcm2837_gpio_clr_multi(mask);
}

/* Set the state of a all 32 outputs in the mask to the values in value */
void bcm2837_gpio_write_mask(uint32_t value, uint32_t mask)
{
    bcm2837_gpio_set_multi(value & mask);
    bcm2837_gpio_clr_multi((~value) & mask);
}
...
void bcm2837_spi_setBitOrder(uint8_t __attribute__((unused)) order)
{
    /* BCM2837_SPI_BIT_ORDER_MSBFIRST is the only one suported by SPI0 */
}
...
void bcm2837_spi_setDataMode(uint8_t mode)
{
    ...
    bcm2837_peri_set_bits(paddr, mode << 2, BCM2837_SPI0_CS_CPOL | BCM2837_SPI0_CS_CPHA);
}
...
uint8_t bcm2837_spi_transfer(uint8_t value)
{
    ...
    */
    ...
    bcm2837_peri_write_nb(fifo, value);
...
void bcm2837_spi_transfernb(char* tbuf, char* rbuf, uint32_t len)
{
    ...
    */
    ...
    while((TXCnt < len)||(RXCnt < len))
    {
        ...
        {
           bcm2837_peri_write_nb(fifo, tbuf[TXCnt]);
           TXCnt++;
        }
        /* Rx fifo not empty, so get the next received bytes */
        while(((bcm2837_peri_read(paddr) & BCM2837_SPI0_CS_RXD))&&( RXCnt < len ))
        {
           rbuf[RXCnt] = bcm2837_peri_read_nb(fifo);
           RXCnt++;
        }
    }
    /* Wait for DONE to be set */
...
void bcm2837_spi_writenb(char* tbuf, uint32_t len)
{
    volatile uint32_t* paddr = bcm2837_spi0 + BCM2837_SPI0_CS/4;
    volatile uint32_t* fifo = bcm2837_spi0 + BCM2837_SPI0_FIFO/4;
    uint32_t i;
...
// Answer: an ISR is required to issue the required memory barriers.
    */
	...
	/* Write to FIFO, no barrier */
	bcm2837_peri_write_nb(fifo, tbuf[i]);
...
void bcm2837_i2c_begin(void)
{
    uint16_t cdiv;

#ifdef I2C_V1
    volatile uint32_t* paddr = bcm2837_bsc0 + BCM2837_BSC_DIV/4;
    /* Set the I2C/BSC0 pins to the Alt 0 function to enable I2C access on them */
    bcm2837_gpio_fsel(RPI_GPIO_P1_03, BCM2837_GPIO_FSEL_ALT0); /* SDA */
    bcm2837_gpio_fsel(RPI_GPIO_P1_05, BCM2837_GPIO_FSEL_ALT0); /* SCL */
#else
    volatile uint32_t* paddr = bcm2837_bsc1 + BCM2837_BSC_DIV/4;
    /* Set the I2C/BSC1 pins to the Alt 0 function to enable I2C access on them */
    bcm2837_gpio_fsel(RPI_V2_GPIO_P1_03, BCM2837_GPIO_FSEL_ALT0); /* SDA */
    bcm2837_gpio_fsel(RPI_V2_GPIO_P1_05, BCM2837_GPIO_FSEL_ALT0); /* SCL */
#endif    
...
void bcm2837_i2c_end(void)
{
    ...
    bcm2837_gpio_fsel(RPI_GPIO_P1_03, BCM2837_GPIO_FSEL_INPT); /* SDA */
    bcm2837_gpio_fsel(RPI_GPIO_P1_05, BCM2837_GPIO_FSEL_INPT); /* SCL */
#else
    /* Set all the I2C/BSC1 pins back to input */
    bcm2837_gpio_fsel(RPI_V2_GPIO_P1_03, BCM2837_GPIO_FSEL_INPT); /* SDA */
    bcm2837_gpio_fsel(RPI_V2_GPIO_P1_05, BCM2837_GPIO_FSEL_INPT); /* SCL */
#endif
}
...
#ifdef I2C_V1
    volatile uint32_t* paddr = bcm2837_bsc0 + BCM2837_BSC_A/4;
#else	
    volatile uint32_t* paddr = bcm2837_bsc1 + BCM2837_BSC_A/4;
#endif
...
{
#ifdef I2C_V1
    volatile uint32_t* dlen    = bcm2837_bsc0 + BCM2837_BSC_DLEN/4;
    volatile uint32_t* fifo    = bcm2837_bsc0 + BCM2837_BSC_FIFO/4;
    volatile uint32_t* status  = bcm2837_bsc0 + BCM2837_BSC_S/4;
    volatile uint32_t* control = bcm2837_bsc0 + BCM2837_BSC_C/4;
#else
    volatile uint32_t* dlen    = bcm2837_bsc1 + BCM2837_BSC_DLEN/4;
    volatile uint32_t* fifo    = bcm2837_bsc1 + BCM2837_BSC_FIFO/4;
    volatile uint32_t* status  = bcm2837_bsc1 + BCM2837_BSC_S/4;
    volatile uint32_t* control = bcm2837_bsc1 + BCM2837_BSC_C/4;
#endif    
    ...
    bcm2837_peri_write_nb(control, BCM2837_BSC_C_I2CEN | BCM2837_BSC_C_ST);
    
    /* Transfer is over when BCM2837_BSC_S_DONE */
    while(!(bcm2837_peri_read_nb(status) & BCM2837_BSC_S_DONE ))
    {
        while ( remaining && (bcm2837_peri_read_nb(status) & BCM2837_BSC_S_TXD ))
    	{
	    /* Write to FIFO, no barrier */
        	bcm2837_peri_write_nb(fifo, buf[i]);
        	i++;
...
uint8_t bcm2837_i2c_read(char* buf, uint32_t len)
{
    ...
    while (!(bcm2837_peri_read_nb(status) & BCM2837_BSC_S_DONE))
    {
        ...
        while (bcm2837_peri_read_nb(status) & BCM2837_BSC_S_RXD)
    	{
...
{   
    ...
    bcm2837_peri_write_nb(status, BCM2837_BSC_S_CLKT | BCM2837_BSC_S_ERR | BCM2837_BSC_S_DONE);
    /* Set Data Length */
    bcm2837_peri_write_nb(dlen, 1);
    /* Enable device and start transfer */
    bcm2837_peri_write_nb(control, BCM2837_BSC_C_I2CEN);
    bcm2837_peri_write_nb(fifo, regaddr[0]);
    bcm2837_peri_write_nb(control, BCM2837_BSC_C_I2CEN | BCM2837_BSC_C_ST);
    
    /* poll for transfer has started */
    while ( !( bcm2837_peri_read_nb(status) & BCM2837_BSC_S_TA ) )
    {
...
{   
    ...
    /* Set Data Length */
    bcm2837_peri_write_nb(dlen, cmds_len);
    ...
    /* Send a repeated start with read bit set in address */
    bcm2837_peri_write_nb(dlen, buf_len);
    bcm2837_peri_write_nb(control, BCM2837_BSC_C_I2CEN | BCM2837_BSC_C_ST  | BCM2837_BSC_C_READ );
...
uint64_t bcm2837_st_read(void)
{
    volatile uint32_t* paddr;
    uint32_t hi, lo;
    uint64_t st;
    paddr = bcm2837_st + BCM2837_ST_CHI/4;
    hi = bcm2837_peri_read(paddr);

    paddr = bcm2837_st + BCM2837_ST_CLO/4;
    lo = bcm2837_peri_read(paddr);
    
    paddr = bcm2837_st + BCM2837_ST_CHI/4;
    st = bcm2837_peri_read(paddr);
...
void bcm2837_st_delay(uint64_t offset_micros, uint64_t micros)
{
    uint64_t compare = offset_micros + micros;

    while(bcm2837_st_read() < compare)
	;
}

/* PWM */

void bcm2837_pwm_set_clock(uint32_t divisor)
{
    /* From Gerts code */
    divisor &= 0xfff;
    /* Stop PWM clock */
    bcm2837_peri_write(bcm2837_clk + BCM2837_PWMCLK_CNTL, BCM2837_PWM_PASSWRD | 0x01);
    bcm2837_delay(110); /* Prevents clock going slow */
    /* Wait for the clock to be not busy */
    while ((bcm2837_peri_read(bcm2837_clk + BCM2837_PWMCLK_CNTL) & 0x80) != 0)
	bcm2837_delay(1); 
    /* set the clock divider and enable PWM clock */
    bcm2837_peri_write(bcm2837_clk + BCM2837_PWMCLK_DIV, BCM2837_PWM_PASSWRD | (divisor << 12));
    bcm2837_peri_write(bcm2837_clk + BCM2837_PWMCLK_CNTL, BCM2837_PWM_PASSWRD | 0x11); /* Source=osc and enable */
}

void bcm2837_pwm_set_mode(uint8_t channel, uint8_t markspace, uint8_t enabled)
{
  uint32_t control = bcm2837_peri_read(bcm2837_pwm + BCM2837_PWM_CONTROL);

  if (channel == 0)
    {
      if (markspace)
	control |= BCM2837_PWM0_MS_MODE;
      else
	control &= ~BCM2837_PWM0_MS_MODE;
      if (enabled)
	control |= BCM2837_PWM0_ENABLE;
      else
	control &= ~BCM2837_PWM0_ENABLE;
    }
  else if (channel == 1)
    {
      if (markspace)
	control |= BCM2837_PWM1_MS_MODE;
      else
	control &= ~BCM2837_PWM1_MS_MODE;
      if (enabled)
	control |= BCM2837_PWM1_ENABLE;
      else
	control &= ~BCM2837_PWM1_ENABLE;
    }

  /* If you use the barrier here, wierd things happen, and the commands dont work */
  bcm2837_peri_write_nb(bcm2837_pwm + BCM2837_PWM_CONTROL, control);
  /*  bcm2837_peri_write_nb(bcm2837_pwm + BCM2837_PWM_CONTROL, BCM2837_PWM0_ENABLE | BCM2837_PWM1_ENABLE | BCM2837_PWM0_MS_MODE | bcm2837_PWM1_MS_MODE); */

}

void bcm2837_pwm_set_range(uint8_t channel, uint32_t range)
{
  if (channel == 0)
      bcm2837_peri_write_nb(bcm2837_pwm + BCM2837_PWM0_RANGE, range);
  else if (channel == 1)
      bcm2837_peri_write_nb(bcm2837_pwm + BCM2837_PWM1_RANGE, range);
}

void bcm2837_pwm_set_data(uint8_t channel, uint32_t data)
{
  if (channel == 0)
      bcm2837_peri_write_nb(bcm2837_pwm + BCM2837_PWM0_DATA, data);
  else if (channel == 1)
      bcm2837_peri_write_nb(bcm2837_pwm + BCM2837_PWM1_DATA, data);
}
...
{
    void *mem;
    errno = posix_memalign(&mem, BCM2837_PAGE_SIZE, size);
...
static void *mapmem(const char *msg, size_t size, int fd, off_t off)
{
    ...
    if (map == MAP_FAILED)
	fprintf(stderr, "bcm2837_init: %s mmap failed: %s\n", msg, strerror(errno));
    return map;
}
...
int bcm2837_init(void)
{
    ...
    if (debug) 
    {
        bcm2837_peripherals = (uint32_t*)BCM2837_PERI_BASE;

	bcm2837_pads = bcm2837_peripherals + BCM2837_GPIO_PADS/4;
	bcm2837_clk  = bcm2837_peripherals + BCM2837_CLOCK_BASE/4;
	bcm2837_gpio = bcm2837_peripherals + BCM2837_GPIO_BASE/4;
	bcm2837_pwm  = bcm2837_peripherals + BCM2837_GPIO_PWM/4;
	bcm2837_spi0 = bcm2837_peripherals + BCM2837_SPI0_BASE/4;
	bcm2837_bsc0 = bcm2837_peripherals + BCM2837_BSC0_BASE/4;
	bcm2837_bsc1 = bcm2837_peripherals + BCM2837_BSC1_BASE/4;
	bcm2837_st   = bcm2837_peripherals + BCM2837_ST_BASE/4;
	return 1; /* Success */
    }
    ...
    */
	...
	if (fread(buf, 1, sizeof(buf), fp) == sizeof(buf))
	  bcm2837_peripherals_base = (uint32_t *)(buf[0] << 24 | buf[1] << 16 | buf[2] << 8 | buf[3] << 0);
	fseek(fp, BMC2835_RPI2_DT_PERI_SIZE_OFFSET, SEEK_SET);
	if (fread(buf, 1, sizeof(buf), fp) == sizeof(buf))
	  bcm2837_peripherals_size = (buf[0] << 24 | buf[1] << 16 | buf[2] << 8 | buf[3] << 0);
	fclose(fp);
    }
    /* else we are prob on RPi 1 with BCM2837, and use the hardwired defaults */
    ...
    {
	fprintf(stderr, "bcm2837_init: Unable to open /dev/mem: %s\n",
		strerror(errno)) ;
	goto exit;
    }
	
    /* Base of the peripherals block is mapped to VM */
    bcm2837_peripherals = mapmem("gpio", bcm2837_peripherals_size, memfd, (uint32_t)bcm2837_peripherals_base);
    if (bcm2837_peripherals == MAP_FAILED) goto exit;

    /* Now compute the base addresses of various peripherals, 
    // which are at fixed offsets within the mapped peripherals block
    // Caution: bcm2837_peripherals is uint32_t*, so divide offsets by 4
    */
    bcm2837_gpio = bcm2837_peripherals + BCM2837_GPIO_BASE/4;
    bcm2837_pwm  = bcm2837_peripherals + BCM2837_GPIO_PWM/4;
    bcm2837_clk  = bcm2837_peripherals + BCM2837_CLOCK_BASE/4;
    bcm2837_pads = bcm2837_peripherals + BCM2837_GPIO_PADS/4;
    bcm2837_spi0 = bcm2837_peripherals + BCM2837_SPI0_BASE/4;
    bcm2837_bsc0 = bcm2837_peripherals + BCM2837_BSC0_BASE/4; /* I2C */
    bcm2837_bsc1 = bcm2837_peripherals + BCM2837_BSC1_BASE/4; /* I2C */
    bcm2837_st   = bcm2837_peripherals + BCM2837_ST_BASE/4;
...
int bcm2837_close(void)
{
    if (debug) return 1; /* Success */

    unmapmem((void**) &bcm2837_peripherals, bcm2837_peripherals_size);
    bcm2837_peripherals = MAP_FAILED;
    bcm2837_gpio = MAP_FAILED;
    bcm2837_pwm  = MAP_FAILED;
    bcm2837_clk  = MAP_FAILED;
    bcm2837_pads = MAP_FAILED;
    bcm2837_spi0 = MAP_FAILED;
    bcm2837_bsc0 = MAP_FAILED;
    bcm2837_bsc1 = MAP_FAILED;
    bcm2837_st   = MAP_FAILED;
    return 1; /* Success */
}    
...
int main(int argc, char **argv)
{
    ...
    */
    bcm2837_gpio_fsel(RPI_GPIO_P1_11, BCM2837_GPIO_FSEL_OUTP);
    /* Set RPI pin P1-15 to be an input */
    bcm2837_gpio_fsel(RPI_GPIO_P1_15, BCM2837_GPIO_FSEL_INPT);
    /*  with a pullup */
    bcm2837_gpio_set_pud(RPI_GPIO_P1_15, BCM2837_GPIO_PUD_UP);
    /* And a low detect enable */
    bcm2837_gpio_len(RPI_GPIO_P1_15);
    /* and input hysteresis disabled on GPIOs 0 to 27 */
    bcm2837_gpio_set_pad(BCM2837_PAD_GROUP_GPIO_0_27, BCM2837_PAD_SLEW_RATE_UNLIMITED|BCM2837_PAD_DRIVE_8mA);
    ...
    {
	/* Turn it on */
	bcm2837_gpio_write(RPI_GPIO_P1_11, HIGH);
	
	/* wait a bit */
	bcm2837_delay(500);
	
	/* turn it off */
	bcm2837_gpio_write(RPI_GPIO_P1_11, LOW);
    ...
    {
	/* Read some data */
	uint8_t value = bcm2837_gpio_lev(RPI_GPIO_P1_15);
    ...
    */
    while (1)
    {
	if (bcm2837_gpio_eds(RPI_GPIO_P1_15))
	{
	    /* Now clear the eds flag by setting it to 1 */
	    bcm2837_gpio_set_eds(RPI_GPIO_P1_15);
