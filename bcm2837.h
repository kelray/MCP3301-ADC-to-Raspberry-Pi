/* bcm2837.h
   ...
   $Id: bcm2837.h,v 1.18 2015/03/08 22:17:20 mikem Exp $
*/
...
/*! \mainpage C library for Broadcom BCM 2835 as used in Raspberry Pi
  ...
  from http:  www.airspayce.com/mikem/bcm2837/bcm2837-1.42.tar.gz
  You can find the latest version at http://www.airspayce.com/mikem/bcm2837
  
  Several example programs are provided.
  
  Based on data in http://elinux.org/RPi_Low-level_peripherals and 
  http://www.raspberrypi.org/wp-content/uploads/2012/02/BCM2837-ARM-Peripherals.pdf
  and http://www.scribd.com/doc/101830961/GPIO-Pads-Control2
  
  You can also find online help and discussion at http://groups.google.com/group/bcm2837
  ...
  and Occidentalisv01
  CAUTION: it has been observed that when detect enables such as bcm2837_gpio_len() 
  ...
  If you must use bcm2837_gpio_len() and friends, make sure you disable the pins with 
  bcm2837_gpio_clr_len() and friends after use. 
  ...
  \code
  # download the latest version of the library, say bcm2837-1.xx.tar.gz, then:
  tar zxvf bcm2837-1.xx.tar.gz
  cd bcm2837-1.xx
  ./configure
  make
  sudo make check
  ...
  The functions bcm2837_peri_read(), bcm2837_peri_write() and bcm2837_peri_set_bits() 
  ...
  of the BCM2837 ARM Peripherals manual. 
  ...
  On RPI 2, the peripheral addresses are different and the bcm2837 library gets them 
  ...
  BCM 2835 chip pin numbering. http://elinux.org/RPi_BCM2837_GPIOs
  ...
  The bcm2837_spi_* functions allow you to control the BCM 2835 SPI0 interface, 
  ...
  When bcm2837_spi_begin() is called it changes the bahaviour of the SPI interface pins from their 
  default GPIO behaviour in order to support SPI. While SPI is in use, you will not be able 
  to control the state of the SPI pins through the usual bcm2837_spi_gpio_write().
  When bcm2837_spi_end() is called, the SPI pins will all revert to inputs, and can then be
  configured and controled with the usual bcm2837_gpio_* calls.
  ...
  The bcm2837_i2c_* functions allow you to control the BCM 2835 BSC interface,
  ...
  The BCM2837 supports hardware PWM on a limited subset of GPIO pins. This bcm2837 library provides 
  functions for configuring and controlling PWM output on these pins.
  
  The BCM2837 contains 2 independent PWM channels (0 and 1), each of which be connnected to a limited subset of 
  ...
  bcm2837_pwm_set_clock(). Each channel can be separately enabled with bcm2837_pwm_set_mode().
  The average output of the PWM channel is determined by the ratio of DATA/RANGE for that channel.
  Use bcm2837_pwm_set_range() to set the range and bcm2837_pwm_set_data() to set the data in that ratio
  ...
  a 19.2MHz clock. You can set any divider, but some common ones are provided by the BCM2837_PWM_CLOCK_DIVIDER_*
  values of \ref bcm2837PWMClockDivider.
  ...
  The bcm2837 is a library for user programs (i.e. they run in 'userland'). 
  ...
  bcm2837_delay() and bcm2837_delayMicroseconds() will return after exactly the time requested. 
  ...
  http://search.cpan.org/~mikem/Device-BCM2837-1.9/lib/Device/BCM2837.pm
  Matthew Baker has kindly made Python bindings available at:
  https:  github.com/mubeta06/py-libbcm2837
  Gary Marks has created a Serial Peripheral Interface (SPI) command-line utility 
  for Raspberry Pi, based on the bcm2837 library. The 
  ...
  \version 1.3 Added bcm2837_spi_transfern()

  \version 1.4 Fixed a problem that prevented SPI CE1 being used. Reported by David Robinson.

  \version 1.5 Added bcm2837_close() to deinit the library. Suggested by C?sar Ortiz

  \version 1.6 Document testing on 2012-07-15-wheezy-raspbian and Occidentalisv01
  Functions bcm2837_gpio_ren(), bcm2837_gpio_fen(), bcm2837_gpio_hen()
  bcm2837_gpio_len(), bcm2837_gpio_aren() and bcm2837_gpio_afen() now 
  changes only the pin specified. Other pins that were already previously
  enabled stay enabled.
  Added  bcm2837_gpio_clr_ren(), bcm2837_gpio_clr_fen(), bcm2837_gpio_clr_hen()
  bcm2837_gpio_clr_len(), bcm2837_gpio_clr_aren(), bcm2837_gpio_clr_afen() 
  to clear the enable for individual pins, suggested by Andreas Sundstrom.

  \version 1.7 Added bcm2837_spi_transfernb to support different buffers for read and write.
  ...
  BCM2837_PAD_HYSTERESIS_ENABLED to 0x08 as per Gert van Loo's doc at
  http://www.scribd.com/doc/101830961/GPIO-Pads-Control2
  Also added a define for the passwrd value that Gert says is needed to
  change pad control settings.

  \version 1.10 Changed the names of the delay functions to bcm2837_delay() 
  and bcm2837_delayMicroseconds() to prevent collisions with wiringPi.
  Macros to map delay()-> bcm2837_delay() and
  Macros to map delayMicroseconds()-> bcm2837_delayMicroseconds(), which
  can be disabled by defining BCM2837_NO_DELAY_COMPATIBILITY
  ...
  Hardware base pointers are now available (after initialisation) externally as bcm2837_gpio
  bcm2837_pwm bcm2837_clk bcm2837_pads bcm2837_spi0.
  ...
  \version 1.15 Added bcm2837_close() to end of examples as suggested by Mark Wolfe.

  \version 1.16 Added bcm2837_gpio_set_multi, bcm2837_gpio_clr_multi and bcm2837_gpio_write_multi
  to allow a mask of pins to be set all at once. Requested by Sebastian Loncar.

  \version 1.17  Added bcm2837_gpio_write_mask. Requested by Sebastian Loncar.

  \version 1.18 Added bcm2837_i2c_* functions. Changes to bcm2837_delayMicroseconds: 
  ...
  \version 1.22 Fixed incorrect definition of BCM2837_GPFEN0 which broke the ability to set 
  falling edge events. Reported by Mark Dootson.

  \version 1.23 Added bcm2837_i2c_set_baudrate and bcm2837_i2c_read_register_rs. 
  Improvements to bcm2837_i2c_read and bcm2837_i2c_write functions
  ...
  \version 1.26 Added missing unmapmem for pads in bcm2837_close to prevent a memory leak. 
  Reported by Hartmut Henkel.

  \version 1.27 bcm2837_gpio_set_pad() no longer needs BCM2837_PAD_PASSWRD: it is
  now automatically included.
  Added suport for PWM mode with bcm2837_pwm_* functions.

  \version 1.28 Fixed a problem where bcm2837_spi_writenb() would have problems with transfers of more than
  64 bytes dues to read buffer filling. Patched by Peter Würtz.

  \version 1.29 Further fix to SPI from Peter Würtz.

  \version 1.30 10 microsecond delays from bcm2837_spi_transfer and bcm2837_spi_transfern for
  ...
  \version 1.34 Added bcm2837_i2c_write_read_rs() which writes an arbitrary number of bytes, 
  ...
  Improvements to bcm2837_st_read to account for possible timer overflow, contributed by 'Ed'.<br>
  Added definitions for Raspberry Pi B+ J8 header GPIO pins.<br>

  \version 1.38 Added bcm2837_regbase for the benefit of C# wrappers, patch by Frank Hommers <br>
  ...
  Fixed a number of compiler errors and warnings that occur when bcm2837.h is included
  in code compiled with -Wall -Woverflow -Wstrict-overflow -Wshadow -Wextra -pedantic.
  Reported by tlhackque.<br>
  Fixed a problem where calling bcm2837_delayMicroseconds loops forever when debug is set. Reported by tlhackque.<br>
  Reinstated use of volatile in 2 functions where there was a danger of lost reads or writes. Reported by tlhackque.<br>
  
  \version 1.41 Added BCM2837_VERSION macro and new function bcm2837_version(); Requested by tlhackque.<br>
...
#define BCM2837_VERSION 10042 /* Version 1.42 */
...
#define BCM2837_HAVE_DMB
#endif

/*! \defgroup constants Constants for passing to and from library functions
  The values here are designed to be passed to various functions in the bcm2837 library.
  @{
*/
...
#define BCM2837_PERI_SIZE               0x01000000
...
*/
extern uint32_t *bcm2837_peripherals_base;
/*! Size of the peripherals block to be mapped */
extern uint32_t bcm2837_peripherals_size;

/*! Virtual memory address of the mapped peripherals block */
extern uint32_t *bcm2837_peripherals;

/*! Base of the ST (System Timer) registers.
  Available after bcm2837_init has been called
*/
extern volatile uint32_t *bcm2837_st;

/*! Base of the GPIO registers.
  Available after bcm2837_init has been called
*/
extern volatile uint32_t *bcm2837_gpio;

/*! Base of the PWM registers.
  Available after bcm2837_init has been called
*/
extern volatile uint32_t *bcm2837_pwm;

/*! Base of the CLK registers.
  Available after bcm2837_init has been called
*/
extern volatile uint32_t *bcm2837_clk;

/*! Base of the PADS registers.
  Available after bcm2837_init has been called
*/
extern volatile uint32_t *bcm2837_pads;

/*! Base of the SPI0 registers.
  Available after bcm2837_init has been called
*/
extern volatile uint32_t *bcm2837_spi0;

/*! Base of the BSC0 registers.
  Available after bcm2837_init has been called
*/
extern volatile uint32_t *bcm2837_bsc0;

/*! Base of the BSC1 registers.
  Available after bcm2837_init has been called
*/
extern volatile uint32_t *bcm2837_bsc1;
...
#define BCM2837_PAGE_SIZE               (4*1024)
...
typedef enum
{
    ...
    BCM2837_SPI_CLOCK_DIVIDER_256   = 256,     /*!< 256 = 1.024us = 976.5625MHz */
...
/* Defines for ST
   GPIO register offsets from BCM2837_ST_BASE.
   ...
   BCM2837_ST_CLO is the System Timer Counter Lower bits register.
   ...
   BCM2837_ST_CHI is the System Timer Counter Upper bits register.
...
*/
#define BCM2837_ST_CS 			0x0000 /*!< System Timer Control/Status */
#define BCM2837_ST_CLO 			0x0004 /*!< System Timer Counter Lower 32 bits */
#define BCM2837_ST_CHI 			0x0008 /*!< System Timer Counter Upper 32 bits */

/*! @} */


/* Defines for PWM, word offsets (ie 4 byte multiples) */
#define BCM2837_PWM_CONTROL 0
#define BCM2837_PWM_STATUS  1
#define BCM2837_PWM_DMAC    2
#define BCM2837_PWM0_RANGE  4
#define BCM2837_PWM0_DATA   5
#define BCM2837_PWM_FIF1    6
#define BCM2837_PWM1_RANGE  8
#define BCM2837_PWM1_DATA   9

/* Defines for PWM Clock, word offsets (ie 4 byte multiples) */
#define BCM2837_PWMCLK_CNTL     40
#define BCM2837_PWMCLK_DIV      41
#define BCM2837_PWM_PASSWRD     (0x5A << 24)  /*!< Password to enable setting PWM clock */

#define BCM2837_PWM1_MS_MODE    0x8000  /*!< Run in Mark/Space mode */
#define BCM2837_PWM1_USEFIFO    0x2000  /*!< Data from FIFO */
#define BCM2837_PWM1_REVPOLAR   0x1000  /*!< Reverse polarity */
#define BCM2837_PWM1_OFFSTATE   0x0800  /*!< Ouput Off state */
#define BCM2837_PWM1_REPEATFF   0x0400  /*!< Repeat last value if FIFO empty */
#define BCM2837_PWM1_SERIAL     0x0200  /*!< Run in serial mode */
#define BCM2837_PWM1_ENABLE     0x0100  /*!< Channel Enable */

#define BCM2837_PWM0_MS_MODE    0x0080  /*!< Run in Mark/Space mode */
#define BCM2837_PWM_CLEAR_FIFO  0x0040  /*!< Clear FIFO */
#define BCM2837_PWM0_USEFIFO    0x0020  /*!< Data from FIFO */
#define BCM2837_PWM0_REVPOLAR   0x0010  /*!< Reverse polarity */
#define BCM2837_PWM0_OFFSTATE   0x0008  /*!< Ouput Off state */
#define BCM2837_PWM0_REPEATFF   0x0004  /*!< Repeat last value if FIFO empty */
#define BCM2837_PWM0_SERIAL     0x0002  /*!< Run in serial mode */
#define BCM2837_PWM0_ENABLE     0x0001  /*!< Channel Enable */

/*! \brief bcm2837PWMClockDivider
...
typedef enum
{
    BCM2837_PWM_CLOCK_DIVIDER_2048  = 2048,    /*!< 2048 = 9.375kHz */
    BCM2837_PWM_CLOCK_DIVIDER_1024  = 1024,    /*!< 1024 = 18.75kHz */
    BCM2837_PWM_CLOCK_DIVIDER_512   = 512,     /*!< 512 = 37.5kHz */
    BCM2837_PWM_CLOCK_DIVIDER_256   = 256,     /*!< 256 = 75kHz */
    BCM2837_PWM_CLOCK_DIVIDER_128   = 128,     /*!< 128 = 150kHz */
    BCM2837_PWM_CLOCK_DIVIDER_64    = 64,      /*!< 64 = 300kHz */
    BCM2837_PWM_CLOCK_DIVIDER_32    = 32,      /*!< 32 = 600.0kHz */
    BCM2837_PWM_CLOCK_DIVIDER_16    = 16,      /*!< 16 = 1.2MHz */
    BCM2837_PWM_CLOCK_DIVIDER_8     = 8,       /*!< 8 = 2.4MHz */
    BCM2837_PWM_CLOCK_DIVIDER_4     = 4,       /*!< 4 = 4.8MHz */
    BCM2837_PWM_CLOCK_DIVIDER_2     = 2,       /*!< 2 = 9.6MHz, fastest you can get */
    BCM2837_PWM_CLOCK_DIVIDER_1     = 1        /*!< 1 = 4.6875kHz, same as divider 4096 */
} bcm2837PWMClockDivider;

/* Historical name compatibility */
#ifndef BCM2837_NO_DELAY_COMPATIBILITY
#define delay(x) bcm2837_delay(x)
#define delayMicroseconds(x) bcm2837_delayMicroseconds(x)
#endif
...
extern "C" {
    ...
    /*! \defgroup init Library initialisation and management
      These functions allow you to intialise and control the bcm2837 library
      @{
    */
    ...
    /*! Initialise the library by opening /dev/mem and getting pointers to the 
      ...
      functions in this library (except bcm2837_set_debug). 
      If bcm2837_init() fails by returning 0, 
      calling any other function may result in crashes or other failures.
      Prints messages to stderr in case of errors.
      \return 1 if successful else 0
    */
    extern int bcm2837_init(void);

    /*! Close the library, deallocating any allocated memory and closing /dev/mem
      \return 1 if successful else 0
    */
    extern int bcm2837_close(void);
    ...
    /*! Sets the debug level of the library.
      ...
      Call this before calling bcm2837_init();
      \param[in] debug The new debug level. 1 means debug
    */
    extern void  bcm2837_set_debug(uint8_t debug);

    /*! Returns the version number of the library, same as BCM2837_VERSION
       \return the current library version number
    */
    extern unsigned int bcm2837_version(void);
    ...
    */
    ...
    /*! Gets the base of a register
      \param[in] regbase You can use one of the common values BCM2837_REGBASE_*
      in \ref bcm2837RegisterBase
      \return the register base
      \sa Physical Addresses
    */
    extern uint32_t* bcm2837_regbase(uint8_t regbase);
    ...
    */
    ...
    /*! Alters a number of bits in a 32 peripheral regsiter.
      ...
      \sa Physical Addresses
    */
    extern void bcm2837_peri_set_bits(volatile uint32_t* paddr, uint32_t value, uint32_t mask);
    /*! @}    end of lowlevel */
    ...
    */
    extern void bcm2837_delay (unsigned int millis);

    /*! Delays for the specified number of microseconds.
      Uses a combination of nanosleep() and a busy wait loop on the BCM2837 system timers,
      However, you are at the mercy of nanosleep(). From the manual for nanosleep():
      If the interval specified in req is not an exact multiple of the granularity  
      underlying  clock  (see  time(7)),  then the interval will be
      rounded up to the next multiple. Furthermore, after the sleep completes, 
    ...
    */
    extern void bcm2837_delayMicroseconds (uint64_t micros);
    ...
    */
    ...
    /*! Start SPI operations.
      ...
      \sa  bcm2837_spi_end()
    */
    extern void bcm2837_spi_begin(void);
    ...
    */
    extern void bcm2837_spi_writenb(char* buf, uint32_t len);
    ...
    */
    ...
    /*! Start I2C operations.
      ...
      \sa  bcm2837_i2c_end()
    */
    extern void bcm2837_i2c_begin(void);
    ...
    */
    ...
    /*! Allows reading from I2C slaves that require a repeated start (without any prior stop)
      ...
      \return reason see \ref bcm2837I2CReasonCodes
    */
    extern uint8_t bcm2837_i2c_read_register_rs(char* regaddr, char* buf, uint32_t len);
    ...
    /*! Allows sending an arbitrary number of bytes to I2C slaves before issuing a repeated
      ...
      \return reason see \ref bcm2837I2CReasonCodes
    */
    extern uint8_t bcm2837_i2c_write_read_rs(char* cmds, uint32_t cmds_len, char* buf, uint32_t buf_len);
    ...
    */
    extern uint64_t bcm2837_st_read(void);

    /*! Delays for the specified number of microseconds with offset.
      \param[in] offset_micros Offset in microseconds
      \param[in] micros Delay in microseconds
    */
    extern void bcm2837_st_delay(uint64_t offset_micros, uint64_t micros);
    ...
    */
    ...
    /*! Sets the PWM clock divisor, 
      ...
      values BCM2837_PWM_CLOCK_DIVIDER_* in \ref bcm2837PWMClockDivider
    */
    extern void bcm2837_pwm_set_clock(uint32_t divisor);
    ...
    */
    extern void bcm2837_pwm_set_mode(uint8_t channel, uint8_t markspace, uint8_t enabled);
    ...
    */
    extern void bcm2837_pwm_set_range(uint8_t channel, uint32_t range);
    
    /*! Sets the PWM pulse ratio to emit to DATA/RANGE, 
      where RANGE is set by bcm2837_pwm_set_range().
      \param[in] channel The PWM channel. 0 or 1.
      \param[in] data Controls the PWM output ratio as a fraction of the range. 
      Can vary from 0 to RANGE.
    */
    extern void bcm2837_pwm_set_data(uint8_t channel, uint32_t data);
...
Broadcom bcm2837. Contributed by Shahrooz Shahparnia.
*/
